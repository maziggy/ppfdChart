...
class PPFDCustomCard extends LitElement {
	...
	set hass(hass) {
		this._hass = hass;
		const entityId = this.entity || 'input_text.grow_ppfd';
		...
		console.log(`Light off state: ${light_off_state.state}`);
		console.log(`Light on state: ${light_on_state.state}`);
		...
		if (this.shadowRoot) {
			...
			// Pass light_on_state and light_off_state directly to calculateDLI
	...
	// Function to calculate DLI (Daily Light Integral) based on current PPFD
	calculateDLI(currentPPFD, onTimeEntity, offTimeEntity) {
		if (onTimeEntity && offTimeEntity) {
			...
			console.log("Parsed onTime:", onTime);
			console.log("Parsed offTime before adjustment:", offTime);

			// Adjust offTime if it’s earlier than onTime, assuming it’s the next day
			if (offTime <= onTime) {
				offTime.setHours(offTime.getHours() + 24);
			}

			console.log("Adjusted offTime:", offTime);

			const lightDurationMs = offTime - onTime;
			this.lhd = Math.round(lightDurationMs / (1000 * 60 * 60)); // Convert to hours

			console.log("Calculated light hours (lhd):", this.lhd);

			this.dli = Math.round((currentPPFD * this.lhd * 3600) / 1000000);
			console.log("Calculated DLI:", this.dli);
